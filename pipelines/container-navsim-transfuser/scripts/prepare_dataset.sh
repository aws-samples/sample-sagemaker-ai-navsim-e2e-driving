...
# ============================================================
# NAVSIM データセット準備スクリプト (Transfuser)
# ============================================================
#
# conda で Python 3.9 環境を自動作成し、navsim devkit をインストールして
# mini split からカメラ画像・LiDAR BEV・EgoStatus・GT 軌跡を抽出する。
#
# 使い方:
#   ./pipelines/container-navsim-transfuser/scripts/prepare_dataset.sh [プロジェクト名]
#
# 前提条件:
#   - conda がインストール済み (SageMaker AI Notebook にはプリインストール)
#   - AWS CLI が設定済み
#   - ディスク容量 20 GB 以上 (カメラ画像 + LiDAR を含むため)
#
# 所要時間の目安:
#   - 全体: 約 140 分
...
#     - Step 3 (特徴量抽出): 約 85 分
#     - Step 4 (データバランシング): 約 5 分
#     - Step 5 (S3 アップロード): 約 5 分
# ============================================================
...
echo "============================================"
echo "NAVSIM データセット準備 (Transfuser)"
echo "============================================"
...
echo ""
echo "⏱️  所要時間の目安: 約 140 分"
echo "    - conda 環境構築:                        約 5 分"
echo "    - OpenScene mini split ダウンロード:      約 40 分"
echo "    - 地図ダウンロード:                       約 1 分"
echo "    - 特徴量抽出:                            約 85 分"
echo "    - データバランシング:                    約 5 分"
echo "    - S3 アップロード:                       約 5 分"
echo ""
...
# -------------------------------------------------------------------------
# Step 1: conda 環境の作成 (EgoStatusMLP と共有)
# -------------------------------------------------------------------------
echo ""
...
# -------------------------------------------------------------------------
# Step 2: mini split データセットをダウンロード (センサーデータ含む)
# -------------------------------------------------------------------------
echo ""
echo "[Step 2/5] mini split データセットをダウンロード (カメラ + LiDAR 含む)..."
...
if [[ -d "${DATASET_DIR}/navsim_logs/mini" ]]; then
    echo "  navsim_logs: ダウンロード済み。スキップします。"
else
...
# maps のダウンロード (navsim_logs とは独立してチェック)
if [[ -d "${DATASET_DIR}/maps" && -n "$(ls -A "${DATASET_DIR}/maps" 2>/dev/null)" ]]; then
    echo "  maps: ダウンロード済み。スキップします。"
else
    echo "  maps をダウンロード中..."
    # まず既存のダウンロードデータからリンクを試行
    for src in "${DOWNLOAD_DIR}/maps" "${DOWNLOAD_DIR}/mini_maps"; do
        if [[ -d "${src}" ]]; then
            ln -sfn "${src}" "${DATASET_DIR}/maps"
            echo "  Linked $(basename "${src}") → ${DATASET_DIR}/maps"
            break
        fi
    done
    # リンクできなかった場合はダウンロード
    if [[ ! -d "${DATASET_DIR}/maps" || -z "$(ls -A "${DATASET_DIR}/maps" 2>/dev/null)" ]]; then
        if [[ -d "${DOWNLOAD_DIR}" && -f "${DOWNLOAD_DIR}/download_maps.sh" ]]; then
            pushd "${DOWNLOAD_DIR}" > /dev/null
            chmod +x ./download_maps.sh
            bash ./download_maps.sh || echo "WARNING: maps download exited with non-zero"
            popd > /dev/null
            for src in "${DOWNLOAD_DIR}/maps" "${DOWNLOAD_DIR}/mini_maps"; do
                if [[ -d "${src}" ]]; then
                    ln -sfn "${src}" "${DATASET_DIR}/maps"
...
# -------------------------------------------------------------------------
echo ""
echo "[Step 3/5] 特徴量を抽出 (カメラ + LiDAR + EgoStatus + 軌跡)..."
CACHE_DIR="${WORK_DIR}/cache/transfuser"
mkdir -p "${CACHE_DIR}"
...
for prefix in train test; do
    if [[ -d "${CACHE_DIR}/${prefix}" ]]; then
        aws s3 cp "${CACHE_DIR}/${prefix}/" "s3://${DATASET_BUCKET}/${CONTAINER_TAG}/${prefix}/" \
...
echo "元のデータバックアップ: ${CACHE_DIR}/train_original (バランシング前)"

echo ""
echo "============================================"
echo "✅ データセット準備完了 (Transfuser)"
echo "============================================"
echo "Train: s3://${DATASET_BUCKET}/${CONTAINER_TAG}/train/"
echo "Test:  s3://${DATASET_BUCKET}/${CONTAINER_TAG}/test/"
echo ""
echo "次のステップ:"
echo "  ./pipelines/scripts/run-pipeline.sh -c container-navsim-transfuser --skip-upload"
