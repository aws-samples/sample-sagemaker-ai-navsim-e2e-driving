...
#   - Removed navsim/nuplan imports; self-defined StateSE2Index / BoundingBox2DIndex
#   - Replaced TrajectorySampling with config.num_poses
#   - Architecture is otherwise identical to the official implementation
#   - それ以外のアーキテクチャは公式と同一
...
class StateSE2Index:
    """nuplan.common.enums.StateSE2Index の代替。"""
    X = 0
    Y = 1
    HEADING = 2
...
class BoundingBox2DIndex:
    """navsim.agents.transfuser.transfuser_features.BoundingBox2DIndex の代替。"""
    X = 0
    Y = 1
    HEADING = 2
    LENGTH = 3
...
class TransfuserModel(nn.Module):
    ...
    def __init__(self, config: TransfuserConfig):
        ...
        self._bev_semantic_head = nn.Sequential(
            nn.Conv2d(config.bev_features_channels, config.bev_features_channels,
                      kernel_size=(3, 3), stride=1, padding=(1, 1), bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(config.bev_features_channels, config.num_bev_classes,
        ...
        self._agent_head = AgentHead(
            num_agents=config.num_bounding_boxes, d_ffn=config.tf_d_ffn, d_model=config.tf_d_model,
        )
    ...
    def forward(self, features: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        lidar_feature = None if self._config.latent else features["lidar_feature"]
        ...
        output.update(self._trajectory_head(trajectory_query))
